...
> **目标：** 在现有 Web 页面中新增「群聊模式」和「拓扑模式」，与现有的「单聊模式」形成三套视图，分别服务于不同场景。群聊模式用于围观所有 agent 的最终发言（如狼人杀），拓扑模式用于监控 agent 工作状态和调用关系。

**架构：** 前端采用「统一状态层 + 三视图切换」架构。后端扩展 WebSocket 支持 `targetAgent` 字段。三种视图共享同一套 WebSocket 数据流，仅渲染方式不同。
...
| **群聊模式**（新增） | 围观群聊 | 所有 agent 的 user/assistant 消息，过滤过程消息 | 狼人杀、多 agent 讨论 |
| **拓扑模式**（新增） | 状态监控 | Agent 头像+状态+连线关系 | 监控工作流、查看调用链路 |
...
```
┌─────────────────────────────────────────────────────────────┐
│ coloop-agent Web    [单聊 | 群聊 | 拓扑]                     │
├──────────┬────────────────────────────────────┬─────────────┤
│          │                                    │             │
│  成员    │         群聊消息区域                │   群成员    │
│  头像    │                                    │   列表      │
│  列表    │  ┌───┐ 我 → dev                    │             │
│  (左)    │  │我 │ 去把那个 bug 修了           │ ⭐ main     │
│          │  └───┘                             │   (群主)    │
│  🤖 dev  │                                    │ 🤖 dev      │
│  🤖 test │  ┌───┐ dev                        │   [工作中]  │
│          │  │🤖 │ 已修复，请验证              │ 🤖 test     │
